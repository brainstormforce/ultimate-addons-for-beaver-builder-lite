...
class UABB_UI_Panels {
	...
	public function __construct() {
		...
		// Enqueue CSS & JS.
		add_action( 'wp_enqueue_scripts', [ $this, 'uabb_panel_css_js' ] );

		add_action( 'wp_footer', [ $this, 'render_live_preview' ], 9 );

		// Render JS & CSS.
		add_filter( 'fl_builder_render_css', [ $this, 'fl_uabb_render_css' ], 10, 3 );
		add_filter( 'fl_builder_render_js', [ $this, 'fl_uabb_render_js' ], 10, 3 );

		// skip brainstorm registration for updater.
		add_filter( 'bsf_skip_author_registration', [ $this, 'uabb_skip_brainstorm_menu' ] );
		add_filter( 'bsf_skip_braisntorm_menu', [ $this, 'uabb_skip_brainstorm_menu' ] );

		// Registration page URL for UABB.
		add_filter( 'bsf_registration_page_url_uabb', [ $this, 'uabb_bsf_registration_page_url' ] );
		add_filter( 'bsf_license_form_heading_uabb', [ $this, 'uabb_bsf_license_form_heading' ], 10, 3 );

		/* Affiliate link override */
		add_filter( 'fl_builder_upgrade_url', [ $this, 'uabb_affiliate_url' ] );
	 ...
	 * @return void
	 */
	public function toggle_uabb_ui(): void {

		// Added ui panel.
		add_action( 'wp_footer', [ $this, 'render_ui' ], 9 );

		// Added buttons in ui panel.
		add_filter( 'fl_builder_ui_bar_buttons', [ $this, 'builder_ui_bar_buttons' ] );

		// Excluded UABB templates.
		add_filter( 'fl_builder_row_templates_data', [ $this, 'uabb_templates_data' ], 50 );
		add_filter( 'fl_builder_module_templates_data', [ $this, 'uabb_templates_data' ], 50 );

		// Added search html in BB 'add content' panel.
		add_action( 'fl_builder_ui_panel_before_rows', [ $this, 'uabb_panel_before_row_layouts' ] );
	}
	...
	public function init(): void {
		add_filter( 'fl_builder_template_selector_data', [ $this, 'uabb_fl_builder_template_selector_data' ], 10, 2 );
	}
	...
	public function uabb_fl_builder_template_selector_data( $template_data, $template ) {
		...
		$template_data['author'] = $template->author ?? '';
	...
	public function uabb_affiliate_url( $url ) {
		...
		return 'https://www.wpbeaverbuilder.com/?fla=713';
	}
	/**
	 * Affiliate link override function
	 *
	...
	public function uabb_bsf_license_form_heading( $form_heading, $license_status_class, $license_status ) {
		...
		if ( $license_status_class === 'bsf-license-not-active-uabb' ) {
			...
			$form_heading .= '<p>Enter your ' . $license_string . ' to enable remote updates and support.</p>';
		}
	...
	public function fl_uabb_render_js( $js, $nodes, $global_settings ) {
		return file_get_contents( BB_ULTIMATE_ADDON_DIR . 'assets/js/uabb-frontend.js' ) . $js;
	}
	 ...
	 * @return void
	 */
	public function uabb_panel_before_row_layouts(): void {
		?>
			<!-- Search Module -->
	 ...
	 * @return array
	 */
	public function uabb_templates_data( $templates, $status = 'exclude' ) {
	...
	public function builder_ui_bar_buttons( $buttons ) {
		...
		$buttons['add-ultimate-presets'] = [
			'label' => __( 'Presets', 'uabb' ),
			'show'  => ( ! $simple_ui && $has_presets ),
		];

		$buttons['add-ultimate-rows'] = [
	 ...
	 * @return void
	 */
	public function render_live_preview(): void {
		if ( FLBuilderModel::is_builder_active() ) {
			/* Live Preview */
	 ...
	 * @return void
	 */
	public function uabb_panel_css_js(): void {
		if ( FLBuilderModel::is_builder_active() ) {
			wp_enqueue_script( 'uabb-panel-js', BB_ULTIMATE_ADDON_URL . 'assets/js/uabb-panel.js', [ 'jquery' ], '', true );
		}
	}
}
