	...
	class BB_Ultimate_Addon_Helper {
		...
		public static function show_rating_notice() {
			...
			if ( ! empty( $posts_created_with_uabb ) && 5 === $posts_created_with_uabb ) {
				...
				$get_posts = $wpdb->prepare( "SELECT count(*) FROM $wpdb->posts wpp INNER JOIN $wpdb->postmeta wppm WHERE wpp.ID = wppm.post_id AND wpp.post_status = '%s' AND wppm.meta_key = %s AND wppm.meta_value REGEXP '|uabb-advanced-menu(.*)|flip-box(.*)|info-list(.*)|info-table(.*)|ribbon(.*)|slide-box(.*)|uabb-button(.*)|spacer-gap(.*)|image-separator(.*)|uabb-separator(.*)|image-icon(.*)|uabb-heading(.*)' LIMIT %d ", 'publish', '_fl_builder_data', 5 );
